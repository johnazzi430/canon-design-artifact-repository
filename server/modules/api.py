...
def persona_table():
    if request.args.get("filter") == "False":
        personas = (
            Persona.query.order_by(Persona.id).filter_by(cust_id=get_cust_id()).all()
        )
        return json.dumps(
            PersonaSchema(exclude=["persona_picture"]).dump(personas, many=True)
        )
    else:
        personas = (
            Persona.query.order_by(Persona.id)
            .filter_by(cust_id=get_cust_id())
            .filter(Persona.archived.is_(False))
            .all()
        )
        return json.dumps(
            PersonaSchema(exclude=["persona_picture"]).dump(personas, many=True)
...
def persona_list():
    ...
    return json.dumps(
        PersonaSchema(only=["id", "name", "title"]).dump(personas, many=True)
    )
...
def persona_table_by_id(id):
    persona = (
        Persona.query.filter(Persona.id == id)
        .options(joinedload("roles"), joinedload("products"))
        .all()
    )
    return json.dumps(
        PersonaSchema(exclude=["persona_picture"]).dump(persona, many=True)
    )
...
def personas_file_get(id):
    if request.args.get("file_id") != None:
        file = PersonaFile.query.filter(
            PersonaFile.id == request.args.get("file_id")
        ).first()
        ...
        files = (
            PersonaFile.query.with_entities(
                PersonaFile.id, PersonaFile.filename, PersonaFile.filetype
            )
            .filter(PersonaFile.source_id == id)
            .all()
        )
        return json.dumps(
            PersonaFileSchema(only=["id", "filename", "filetype"]).dump(
                files, many=True
            )
        )
...
def product_list():
    products = (
        ...
        .filter_by(cust_id=get_cust_id())
        .filter(Product.archived.is_(False))
        .all()
    )
...
def product_table():
    if request.args.get("filter") == "False":
        products = (
            Product.query.order_by(Product.id).filter_by(cust_id=get_cust_id()).all()
        )
...
def product_table_by_id(id):
    products = (
        Product.query.filter(Product.id == id).options(joinedload("personas")).all()
    )
    return json.dumps(ProductSchema().dump(products, many=True))
...
def product_file_get(id):
    if request.args.get("file_id") != None:
        file = ProductFile.query.filter(
            ProductFile.id == request.args.get("file_id")
        ).first()
        response = make_response(file.file)
        ...
        files = (
            ProductFile.query.with_entities(
                ProductFile.id, ProductFile.filename, ProductFile.filetype
            )
            .filter(ProductFile.source_id == id)
            .all()
        )
        return json.dumps(
            ProductFileSchema(only=["id", "filename", "filetype"]).dump(
                files, many=True
            )
        )
...
def insights_get():
    insights = (
        Insight.query.order_by(Insight.id)
        .filter_by(cust_id=get_cust_id())
        .filter(Insight.archived.is_(False))
        .all()
    )
...
def insights_get_by_id(id):
    insight = (
        Insight.query.filter(Insight.id == id)
...
def insights_file_get(id):
    if request.args.get("file_id") != None:
        file = InsightFile.query.filter(
            InsightFile.id == request.args.get("file_id")
        ).first()
        response = make_response(file.file)
        response.headers.set("Content-Type", "multipart/form-data ")
        response.headers.set(
        ...
        files = (
            InsightFile.query.with_entities(
                InsightFile.id, InsightFile.filename, InsightFile.filetype
            )
            .filter(InsightFile.source_id == id)
            .all()
        )
        return json.dumps(
            InsightFileSchema(only=["id", "filename", "filetype"]).dump(
                files, many=True
            )
        )
...
def persona_comments(id):
    persona_comments = PersonaComments.query.filter(
        PersonaComments.source_id == id
    ).all()
...
def product_comments(id):
    product_comments = ProductComments.query.filter(
        ProductComments.source_id == id
    ).all()
...
def insights_comments(id):
    insight_comments = InsightComments.query.filter(
        InsightComments.source_id == id
    ).all()
    ...
    if request.args.get("details") == "True":
        ...
        for play_item in playlist:
            ...
            if source == "persona":
                ...
                data = PersonaSchema(
                    only={"id", "name", "title", "quote", "avatar"}
                ).dump(persona)
            ...
            if source == "insight":
                ...
                data = InsightSchema(
                    only={"id", "title", "description", "experience_vector"}
                ).dump(insight)
                data.update({"source": source})
                response.append(data)
...
def get_user_data():
    if request.args.get("session") == "true":
        ...
        try:
            ...
            return json.dumps(
                UserSchema(only=("username", "user_id", "role", "cust_id")).dump(
                    user, many=True
                )
            )
        except:
